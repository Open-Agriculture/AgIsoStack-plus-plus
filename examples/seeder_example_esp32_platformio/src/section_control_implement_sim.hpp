...
public:
	...
	/// @brief Returns the section state we command, ignoring any fault feedback
	/// @details This drives the physical output (relay): the setpoint in auto mode, or the on-screen
	///          switch in manual mode. It stays asserted even when the machine reports a fault,
	///          because we still command the row on; the fault only affects the reported actual
	///          state, not the command.
	/// @param[in] index The index of the section to get the commanded state for
	/// @returns True if the section is commanded on, otherwise false
	bool get_section_commanded_state(std::uint8_t index) const;

	/// @brief Returns the section's reported actual (working) state by index
	/// @details True only when the section is commanded on AND the machine reports no fault. If a
	///          fault input is active (e.g. motor disconnected or no seed) the row is not actually
	///          working, so this reads false and we report "not working" to the task controller, even
	///          though the commanded state (and thus the relay) stays on.
	...
	/// @brief Sets whether a section is faulted, which disables it while the fault is set
	/// @param[in] index The index of the section to set the fault state for
	/// @param[in] value True to flag the section faulty and force it off, false to clear the fault
	void set_section_fault_state(std::uint8_t index, bool value);

	/// @brief Returns whether a section is currently faulted (disabled)
	/// @param[in] index The index of the section to get the fault state for
	/// @returns True if the section is faulted, otherwise false
	bool get_section_fault_state(std::uint8_t index) const;
...
private:
	...
	std::vector<bool> sectionFaultStates; ///< Stores the faulted/disabled state for each section, e.g. from a digital input (left to right)
