...
// On ESP32 there is typically no mounted filesystem to read "BasePool.iop"
// from, so the object pool is embedded into the firmware image at build time
// (see src/CMakeLists.txt's target_add_binary_data). These linker-generated
// symbols bracket the raw bytes of object_pool/BasePool.iop.
extern "C" const std::uint8_t binary_BasePool_iop_start[] asm("_binary_BasePool_iop_start");
extern "C" const std::uint8_t binary_BasePool_iop_end[] asm("_binary_BasePool_iop_end");
...
std::uint8_t SeederVtApplication::get_number_of_sections() const
{
	return sectionControl.get_number_of_sections();
}

bool SeederVtApplication::get_section_commanded_state(std::uint8_t index) const
{
	return sectionControl.get_section_commanded_state(index);
}

bool SeederVtApplication::get_section_actual_state(std::uint8_t index) const
{
	return sectionControl.get_section_actual_state(index);
}

bool SeederVtApplication::initialize()
{
	objectPool.assign(binary_BasePool_iop_start, binary_BasePool_iop_end);
...
void SeederVtApplication::set_section_fault(std::uint8_t index, bool faulted)
{
	if ((index >= NUMBER_ONSCREEN_SECTIONS) || (sectionControl.get_section_fault_state(index) == faulted))
	{
		return; // Out of range, or unchanged, so don't spam the terminal or the task controller
	}
	sectionControl.set_section_fault_state(index, faulted);

	// The command is untouched, so the relay stays energized. What changes is the reported actual
	// state: get_section_actual_state() now reads this row as "not working", and the cyclic update()
	// repaints its indicator in the fault colour. Tell the TC the actual condensed work state changed
	// so it records the row going not-working promptly rather than waiting for a time-based trigger.
	TCClientInterface.on_value_changed_trigger(static_cast<std::uint16_t>(SectionControlImplementSimulator::ImplementDDOPElementNumbers::BoomElement),
	                                           static_cast<std::uint16_t>(isobus::DataDescriptionIndex::ActualCondensedWorkState1_16));

	std::cout << "Section " << (static_cast<int>(index) + 1) << (faulted ? " reports a fault (e.g. no seed); marking it not working." : " fault cleared.") << std::endl;
}
...
void SeederVtApplication::update_section_objects(std::uint8_t sectionIndex)
{
	...
	if (sectionControl.get_section_fault_state(sectionIndex))
	{
		fillAttribute = solidBlack_FillAttr; // A faulted (disabled) row is flagged black, distinct from red "off"
	}
