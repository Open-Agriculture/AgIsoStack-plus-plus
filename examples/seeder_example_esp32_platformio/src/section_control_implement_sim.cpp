...
SectionControlImplementSimulator::SectionControlImplementSimulator(std::uint8_t value) :
  ...
  sectionFaultStates(value, false)
{
}
...
bool SectionControlImplementSimulator::get_section_commanded_state(std::uint8_t index) const
{
	// The state we command, ignoring any fault feedback. In auto mode this is the task controller's
	// setpoint; in manual mode it's the on-screen section switch. This drives the physical relay, so
	// a faulted row stays commanded (its relay stays energized): the fault only affects what we
	// report back as the actual working state, never the output we drive.
	if (isAutoMode)
	{
...
{
	// We are just simulating here. In a real implement you would read the actual state back from the
	// machine. A fault input means the machine reports a problem (e.g. motor disconnected or no
	// seed), so even though we still command the row on, it is NOT actually working: report it off so
	// it is excluded from the on-count and the task controller sees the row as not working.
	if (sectionFaultStates.at(index))
	{
		return false;
	}
	return get_section_commanded_state(index);
}
...
void SectionControlImplementSimulator::set_section_fault_state(std::uint8_t index, bool value)
{
	sectionFaultStates.at(index) = value;
}

bool SectionControlImplementSimulator::get_section_fault_state(std::uint8_t index) const
{
	return sectionFaultStates.at(index);
}
