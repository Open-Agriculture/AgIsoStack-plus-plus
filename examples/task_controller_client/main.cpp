...
bool create_example_sprayer_ddop(std::shared_ptr<isobus::DeviceDescriptorObjectPool> poolToPopulate, isobus::NAME clientName)
{
	...
	//// Set up Boom
	retVal &= poolToPopulate->add_device_element("Boom", elementCounter++, 0, isobus::task_controller_object::DeviceElementObject::Type::Function, static_cast<std::uint16_t>(SprayerDDOPObjectIDs::SprayBoom));
...
int main()
{
	...
	const isobus::NAMEFilter filterTaskController2(isobus::NAME::NAMEParameters::FunctionInstance, 1);
	const std::vector<isobus::NAMEFilter> tcNameFilters = { filterTaskController, filterTaskController2 };
