#include "socket_can_handler.hpp"
...
SocketCANFrameHandler::SocketCANFrameHandler(const std::string deviceName) :
...
SocketCANFrameHandler::~SocketCANFrameHandler()
{
	close();
...
bool SocketCANFrameHandler::get_is_valid() const
{
	return (-1 != fileDescriptor);
}

void SocketCANFrameHandler::close()
{
	::close(fileDescriptor);
	fileDescriptor = -1;
}

void SocketCANFrameHandler::open()
{
...
bool SocketCANFrameHandler::read_frame(CANHardwareInterface::HardwareInterfaceCANFrame &canFrame)
{
...
bool SocketCANFrameHandler::write_frame(const CANHardwareInterface::HardwareInterfaceCANFrame &canFrame)
{
	struct can_frame txFrame;
	bool retVal = false;
