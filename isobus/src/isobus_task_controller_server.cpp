						...
						case ProcessDataCommands::SetValueAndAcknowledge:
						{
							if (nullptr != get_active_client(rxMessage.get_source_control_function()))
							{
								if (get_active_client(rxMessage.get_source_control_function())->isDDOPActive)
								{
									...
									{
										LOG_DEBUG("[TC Server]: Client %hhu value command for element %u DDI %u and value %d OK.", rxMessage.get_source_control_function()->get_address(), elementNumber, DDI, processVariableValue);
									...
									{
										LOG_ERROR("[TC Server]: Client %hhu value command for element %u DDI %u and value %d failed.", rxMessage.get_source_control_function()->get_address(), elementNumber, DDI, processVariableValue);
